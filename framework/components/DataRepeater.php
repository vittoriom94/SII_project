...
class DataRepeater extends Component
{
    ...
    {
        $dataList = $this->checkForEmptyQuery($dataList);
        $this->arrayValues = $dataList;
    }

    /**
     * Check if a mysqli_result has no rows, so it can return an array with empty values to hide the placeholders.
     * @param array $dataList the dataList passed to the DataRepeater, if it's in array format, just return the dataList
     * @return array The empty mysqli_result or original dataList
     */
    private function checkForEmptyQuery($dataList){
        if($dataList instanceof \mysqli_result && $dataList->num_rows == 0){
            $newDataList = array();
            $fields = $dataList->fetch_fields();
            foreach($fields as $field){
                $fieldName = $field->name;
                $newDataList[$fieldName] = "";
            }
            return array($newDataList);
        } else {
            return $dataList;
        }
